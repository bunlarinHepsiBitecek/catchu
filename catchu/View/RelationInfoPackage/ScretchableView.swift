...
open class ScretchableViewOptions {
...
class ScretchableView: UIView {
    
    open var imageView : UIImageView!
    fileprivate var contentSize = CGSize.zero
    fileprivate var topInset : CGFloat = 0
    fileprivate var options: ScretchableViewOptions!
    ...
    // MARK: Public
    open func scretchableViewSize(headerSize: CGSize, imageSize: CGSize, controller: UIViewController, options: ScretchableViewOptions) {
        ...
        if options.position == ScretchableViewOptions.HeaderPosition.fullScreenTop {
            controller.automaticallyAdjustsScrollViewInsets = false
        }
        
        if options.position == ScretchableViewOptions.HeaderPosition.underNavigationBar {
