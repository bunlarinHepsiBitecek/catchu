...
/*
 ...
 Barometer - Altimeter - Google API : cihazın yerden ne kadar yüksek oldugunu bulacağız. Binaların üzerine sağa sola koymak sıkıntı
 
 Accelerometer - Gyroscope : Kişinin yürüyüp yürümediğini oturup oturmadıgını hesaplayacağız
 
 Magnetometer - Kişinin hangi yöne baktıgını nesnenin arkasında mı önünde mi oldugunu bulup yönlendireceğiz.
 
 SLAM algorithms : Google ARCore, Apple ARKit
 
 occlusion problem
 
 3D Reconstraction deep learning ile derinlik algılama ve objeleri gerçek dünyanın üzerine bindirmek yerine üst üste oturmalarını sağlayacak bir altyapı geliştireceğiz.
 
 Fast Depth Densification for Occlusion-aware Augmented Reality
 
 İçiçe geçen objelerden hızlı derinlik yoğunlaştırma algoritması
